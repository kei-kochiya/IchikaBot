...
class StampsCog(commands.Cog):
    ...
    async def cog_load(self):
        logger.info(
            "StampsCog loaded with %d JP stamps and %d EN stamp names (shared singleton)",
            len(stamp_data.stamps_jp),
            len(stamp_data.stamps_en_names),
        )
    ...
    def create_stamp_embed(self, stamp: dict) -> discord.Embed:
        ...
        embed = discord.Embed(title=f"{name}", color=get_unit_color(char_id))
